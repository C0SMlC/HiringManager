document.addEventListener("DOMContentLoaded", () => {
  ...
  const positionFilter = document.getElementById("positionFilter"); // New position filter
  let allCandidates = [];
    ...
    .then((userData) => {
      ...
      fetch("/candidates?isgetAll=true", {
        ...
        .then((data) => {
          ...
          // Populate "Position Title" dropdown
          const positions = [...new Set(data.map((candidate) => candidate.positionTitle))];
          ...
          function renderCandidates(candidates) {
            ...
            candidates.forEach((candidate) => {
              ...
              row.innerHTML = `
                ...
                <td>${candidate.dateOfPhoneScreen || ""}</td>
          ...
          function applyFilters() {
            ...
            const selectedPosition = positionFilter.value; // Get the selected position
            ...
            filteredCandidates = allCandidates.filter(
              (candidate) =>
                ...
                (selectedPosition === "all" || candidate.positionTitle === selectedPosition) // New position filter
            );
          ...
          positionFilter.addEventListener("change", applyFilters); // Listen to position filter
...
function exportToExcel(filteredData) {
  const headers = [
    ...
    "Status", "Stage", "Interviewer","Date of Phone Screen", "Interview Date", "Date of Offer", "Reason Not Extending", "Notes"
  ];
  ...
  const rows = filteredData.map(candidate => [
    ...
    candidate.stage, candidate.interviewer,candidate.dateOfPhoneScreen || " ", candidate.interviewDate || "",
