...
app.get("/positions/count", (req, res) => {
  const sql = "SELECT SUM(openPositions) AS activePositionsCount FROM OpenPositions WHERE status = 'active'";
  ...
  db.query(sql, (err, results) => {
    ...
    // If no results, default the count to 0
    const activePositionsCount = results[0].activePositionsCount || 0;
    res.status(200).json({ activePositionsCount });
  });
});
